...
class OrderHelper {
  static Branches? getBranch({
    required int id,
    required List<Branches> branchList,
  }) {
    Branches? branches;
  ...
  static double getDeliveryCharge({required OrderModel? orderModel}) =>
      (orderModel?.deliveryCharge ?? 0) + (orderModel?.weightChargeAmount ?? 0);
  ...
  static double getTotalOrderAmount({
    ...
    required double? couponDiscount,
      ...
      (discount + extraDiscount + (couponDiscount ?? 0));

  static TimeSlotModel? getTimeSlot({
    required List<TimeSlotModel>? timeSlotList,
    required int? timeSlotId,
  }) {
    TimeSlotModel? timeSlotModel;
    try {
      timeSlotModel = timeSlotList?.firstWhere(
  ...
  static CartModel? getReorderCartData({
    ...
    if (product != null) {
      ...
      final double basePrice = price ?? product.price ?? 0;
...
enum OrderValue { itemPrice, discount, tax }
