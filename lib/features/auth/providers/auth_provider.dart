...
import 'package:craft_discount_liquors/features/auth/enum/social_login_options_enum.dart';
...
class AuthProvider with ChangeNotifier {
  ...
  Future<SocialLoginModel?> facebookLogin(BuildContext context) async {
    final FacebookLoginProvider facebookLoginProvider =
        Provider.of<FacebookLoginProvider>(context, listen: false);

    await facebookLoginProvider.login();

    if (facebookLoginProvider.result.status == LoginStatus.success) {
      final Map userData = await facebookLoginProvider.facebookAuth.getUserData(
        fields: 'name, email',
      );
  ...
  Future<void> socialLogout() async {
    ...
    if (user?.loginMedium?.toLowerCase() ==
      ...
      await Provider.of<FacebookLoginProvider>(
        Get.context!,
        listen: false,
      ).facebookAuth.logOut();
    }
