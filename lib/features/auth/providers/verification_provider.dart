...
class VerificationProvider with ChangeNotifier {
  ...
  Future<void> sendVerificationCode(
    ...
    if (AuthHelper.isCustomerVerificationEnable(config)) {
      if (type == VerificationType.email.name &&
          AuthHelper.isEmailVerificationEnable(config)) {
        await checkEmail(buildContext, userInput, fromPage);
      } else if (type == VerificationType.phone.name &&
  ...
  Future<ResponseModel> checkPhone(
  ...
  Future<ResponseModel> checkEmail(
    ...
    if (apiResponse.response != null &&
      ...
      String routeName =
          ModalRoute.of(Get.context!)?.settings.name?.split('?').first ?? '';
      ...
      String? errorMessage = ApiCheckerHelper.getError(
        apiResponse,
  ...
  Future<void> firebaseVerifyPhoneNumber(
    ...
    FirebaseAuth.instance.verifyPhoneNumber(
      ...
      codeSent: (String vId, int? resendToken) {
        bool isReplaceRoute =
            ModalRoute.of(context)?.settings.name == RouteHelper.verification;
        ...
        if (fromPage == FromPage.profile.name) {
          ...
          if (isReplaceRoute) {
            RouteHelper.getVerifyRoute(
              phoneNumber,
              isForgetPassword ? FromPage.forget.name : fromPage,
              session: vId,
              action: RouteAction.pushReplacement,
            );
          } else {
  ...
  Future<ResponseModel> verifyPhone(String phone) async {
    final auth.AuthProvider authProvider = Provider.of<auth.AuthProvider>(
      Get.context!,
      listen: false,
    );
    _isLoading = true;
    notifyListeners();
    ApiResponseModel apiResponse = await authRepo!.verifyPhone(
      phone,
      _verificationCode,
    );
    ...
    if (apiResponse.response != null &&
      ...
      String? errorMessage = getTranslated(
  ...
  Future<ResponseModel> verifyToken(String userInput) async {
    ...
    ApiResponseModel apiResponse = await authRepo!.verifyToken(
      userInput,
      _verificationCode,
    );
  ...
  Future<ResponseModel> verifyEmail(String? email) async {
    ...
    _isLoading = true;
    notifyListeners();
    ApiResponseModel apiResponse = await authRepo!.verifyEmail(
      email,
      _verificationCode,
    );
    ...
    if (apiResponse.response != null &&
      ...
      String? errorMessage = ErrorResponseModel.fromJson(
        apiResponse.error,
      ).errors![0].message;
  ...
  Future<ResponseModel> verifyProfileInfo(
    String userInput,
    String type,
    String? session,
  ) async {
    ...
    ApiResponseModel apiResponse = await authRepo!.verifyProfileInfo(
      userInput,
      _verificationCode,
      type,
      session,
    ...
    if (apiResponse.response != null &&
      ...
      String? error = ErrorResponseModel.fromJson(
        apiResponse.error,
      ).errors![0].message;
  ...
  Future<(ResponseModel?, String?)> verifyPhoneForOtp(String phone) async {
    ...
    notifyListeners();
    ApiResponseModel apiResponse = await authRepo!.verifyOtp(
      phone,
      _verificationCode,
    );
    ResponseModel? responseModel;
