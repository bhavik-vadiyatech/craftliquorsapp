...
import 'package:craft_discount_liquors/features/auth/domain/models/social_login_model.dart';
...
class AuthRepo {
  ...
  Future<ApiResponseModel> registerWithOtp(
    String name, {
    String? email,
  ...
  Future<ApiResponseModel> registerWithSocialMedia(
    String name, {
  ...
  Future<ApiResponseModel> verifyProfileInfo(
    ...
    try {
      Response response = await dioClient!.post(
        AppConstants.verifyProfileInfo,
        data: {
          "email_or_phone": userInput,
          "token": token,
          "type": type,
  ...
  Future<ApiResponseModel> checkPhone(String phone) async {
    try {
      Response response = await dioClient!.post(
        AppConstants.checkPhoneUri + phone,
        data: {"phone": phone},
      );
      return ApiResponseModel.withSuccess(response);
    } catch (e) {
  ...
  Future<ApiResponseModel> verifyToken(String? email, String token) async {
    try {
      Response response = await dioClient!.post(
        AppConstants.verifyTokenUri,
        data: {"email": email, "email_or_phone": email, "reset_token": token},
      );
      return ApiResponseModel.withSuccess(response);
    } catch (e) {
      return ApiResponseModel.withError(ApiErrorHandler.getMessage(e));
  ...
  Future<ApiResponseModel> updateToken() async {
    try {
      ...
      await subscribeToAppTopic(deviceToken);
  ...
  Future<void> subscribeTokenToTopic({
    required String token,
    required String topic,
  }) async {
    await dioClient?.post(
      AppConstants.subscribeToTopic,
      data: {"token": token, "topic": topic},
    );
  }

  Future<void> subscribeToAppTopic(String deviceToken) async {
    if (!kIsWeb) {
