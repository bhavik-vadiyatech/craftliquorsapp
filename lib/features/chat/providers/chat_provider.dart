...
class ChatProvider extends ChangeNotifier {
  ...
  Future<void> getDeliveryManMessages(String orderId) async {
    ApiResponseModel apiResponse = await chatRepo!.getDeliveryManMessage(
      orderId,
      1,
    );
    if (apiResponse.response?.data['messages'] != {} &&
        apiResponse.response?.statusCode == 200) {
      _messageList = [];
      _messageList?.addAll(
        ChatModel.fromJson(apiResponse.response!.data).messages!,
      );
    } else {
    ...
    if (isRemove) {
      ...
      if (pickedImages.isNotEmpty) {
        ...
        for (final XFile image in pickedImages) {
          final bool alreadyAdded = _chatImage!.any(
        ...
        _imageFiles = List<XFile>.from(_chatImage!);
  ...
  void removeImage(int index) {
    ...
    _imageFiles = List<XFile>.from(chatImage!);
    notifyListeners();
  }
  ...
  Future<http.StreamedResponse> sendMessageToDeliveryMan(
    ...
    _isLoading = true;
    http.StreamedResponse response = await chatRepo!.sendMessageToDeliveryMan(
      message,
      file,
      orderId,
      token,
  ...
  Future<http.StreamedResponse> sendMessage(
    ...
    _isLoading = true;
    if (senderType == "admin") {
      response = await chatRepo!.sendMessageToAdmin(
        message,
        _chatImage!,
        token,
      );
    } else {
      response = await chatRepo!.sendMessageToDeliveryMan(
        message,
        _chatImage!,
        orderId,
        token,
    ...
    if (response.statusCode == 200) {
      ...
      showCustomSnackBarHelper(
        await ApiCheckerHelper.getStreamedResponseError(response),
      );
    }
