...
import 'package:craft_discount_liquors/features/wallet_and_loyalty/domain/models/wallet_model.dart';
...
class WalletAndLoyaltyProvider with ChangeNotifier {
  ...
  List<String> walletNoteList = const [
    ...
    if (!_offsetList.contains(offset)) {
      ...
      if (apiResponse.response != null &&
        ...
        _transactionList!.addAll(
          WalletModel.fromJson(apiResponse.response!.data).data!,
        );
  ...
  Future<bool> pointToWallet(int point, bool fromWallet) async {
    ...
    _isLoading = true;
    notifyListeners();
    ApiResponseModel apiResponse = await walletRepo!.pointToWallet(
      point: point,
    );
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
  ...
  Future<String?> getAddFundWalletUrl(
