...
import 'package:craft_discount_liquors/helper/wallet_helper.dart';
...
import 'package:craft_discount_liquors/features/wallet_and_loyalty/widgets/bonus_slider_widget.dart';
import 'package:craft_discount_liquors/features/wallet_and_loyalty/widgets/wallet_card_widget.dart';
import 'package:craft_discount_liquors/features/wallet_and_loyalty/widgets/wallet_history_list_widget.dart';
import 'package:craft_discount_liquors/features/wallet_and_loyalty/widgets/wallet_uses_manual_widget.dart';
...
class _WalletScreenState extends State<WalletScreen> {
  ...
  final bool _isLoggedIn = Provider.of<AuthProvider>(
    Get.context!,
    listen: false,
  ).isLoggedIn();
  ...
  void initState() {
    super.initState();
    final walletProvide = Provider.of<WalletAndLoyaltyProvider>(
      context,
      listen: false,
    );
      ...
      if (widget.status != null && widget.status!.contains('success')) {
        if (!kIsWeb ||
          ...
          showCustomSnackBarHelper(
            getTranslated('add_fund_successful', Get.context!),
            isError: false,
          );
        }
      } else if (widget.status != null && widget.status!.contains('fail')) {
        showCustomSnackBarHelper(
          getTranslated('add_fund_failed', Get.context!),
        );
      }
    });
            ...
            return RefreshIndicator(
              ...
              child: _isLoggedIn
                  ? configModel != null && configModel.walletStatus!
                        ...
                        : NoDataWidget(
                            title: getTranslated('not_found', context),
                          )
                  : const NotLoggedInWidget(),
            );
          },
...
class _WalletTitleView extends StatelessWidget {
  const _WalletTitleView({required this.entryList});
            ...
            builder: (context, walletProvider, _) {
              return Stack(
                children: [
                  ...
                  if (walletProvider.type != 'all')
                    Positioned(
                      right: 2,
                      top: 2,
                      child: ActiveStatusIndicatorWidget(),
                    ),
                ],
              );
            },
