...
import 'package:craft_discount_liquors/features/wallet_and_loyalty/domain/models/wallet_bonus_model.dart';
...
class _BonusSliderWidgetState extends State<BonusSliderWidget> {
  ...
  Widget build(BuildContext context) {
    final bool isAddFundActive = Provider.of<SplashProvider>(
      context,
      listen: false,
    ).configModel!.isAddFundToWallet!;

    return isAddFundActive
        ? Consumer<WalletAndLoyaltyProvider>(
            builder: (context, walletProvider, _) {
              return Column(
                mainAxisSize: MainAxisSize.min,
                children: [
                  if (walletProvider.walletBonusList != null &&
                      walletProvider.walletBonusList!.isNotEmpty)
                    CarouselSlider.builder(
                      ...
                      itemBuilder: (ctx, index, realIdx) => BonusItemView(
                        walletBonusModel:
                            walletProvider.walletBonusList![index],
                      ),
                    ),
                  ...
                  if (walletProvider.walletBonusList != null)
                    Row(
                      ...
                      children: walletProvider.walletBonusList!.map((b) {
        ...
        ? Consumer<WalletAndLoyaltyProvider>(
            builder: (context, walletProvider, _) {
              return CustomSingleChildListWidget(
                itemCount: walletProvider.walletBonusList == null
                    ? 3
                    : walletProvider.walletBonusList!.length,
                scrollDirection: Axis.horizontal,
                itemBuilder: (index) => walletProvider.walletBonusList == null
                    ? const WebBonusShimmer()
                    : BonusItemView(
                        walletBonusModel:
                            walletProvider.walletBonusList![index],
                      ),
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        image: const DecorationImage(
          image: AssetImage(Images.walletBanner),
          fit: BoxFit.contain,
      ...
      child: Column(
        ...
        children: [
          ...
          if (walletBonusModel.endDate != null)
            Text(
              '${getTranslated('valid_till', context)} ${DateConverterHelper.estimatedDate(walletBonusModel.endDate!)}',
          ...
          Flexible(
            child: Text(
              walletBonusModel.minimumAddAmount != null &&
                      walletBonusModel.minimumAddAmount! > 0
                  ? '${getTranslated('add_minimum', context)} ${PriceConverterHelper.convertPrice(context, walletBonusModel.minimumAddAmount)} ${getTranslated('and_enjoy_bonus', context)} '
                        '${walletBonusModel.bonusType == 'percentage' ? '${walletBonusModel.bonusAmount} %' : PriceConverterHelper.convertPrice(context, walletBonusModel.bonusAmount)}'
                  : '${getTranslated('add_fund_to_wallet_add_enjoy', context)} ${walletBonusModel.bonusType == 'percentage' ? '${walletBonusModel.bonusAmount} %' : PriceConverterHelper.convertPrice(context, walletBonusModel.bonusAmount)} ${getTranslated('more', context)}',
              maxLines: 1,
              overflow: TextOverflow.ellipsis,
              style: poppinsRegular.copyWith(
                color: Theme.of(context).primaryColor,
