...
class _AddFundDialogueWidgetState extends State<AddFundDialogueWidget> {
  ...
  void initState() {
    ...
    Provider.of<OrderProvider>(
      context,
      listen: false,
    ).changePaymentMethod(isUpdate: false, isClear: true);
  }
                          ...
                          CustomButtonWidget(
                            ...
                            onPressed: () async {
                              ...
                              if (inputAmountController.text.isEmpty) {
                                ...
                                final paymentUrl = await walletProvider
                                    .getAddFundWalletUrl(
                                      context,
                                      customerId:
                                          profileProvider.userInfoModel?.id,
                                      paymentMethod:
                                          orderProvider.paymentMethod?.getWay,
...
class PaymentMethodView extends StatelessWidget {
  ...
  const PaymentMethodView({
    ...
    this.toolTip,
    required this.hideDigital,
    required this.hideOffline,
    this.isFromAddFund = false,
    this.selectedPaymentMethod,
    this.callBack,
    this.selectedOfflineMethod,
    this.selectedOfflineValue,
  });
        ...
        children: [
          ...
          ListView.builder(
            ...
            itemBuilder: (context, index) {
              bool isSelected =
                  paymentList[index].getWayTitle == selectedPaymentMethod;
              bool isOffline = paymentList[index].type == 'offline';

              return Opacity(
                opacity:
                    (isOffline && hideOffline) || (!isOffline && hideDigital)
                    ? 0.4
                    : 1,
                child: InkWell(
                  onTap:
                      (isSelected && isOffline) ||
                          (isOffline && hideOffline) ||
                          (!isOffline && hideDigital)
                      ? null
                      : () => onTap(index),
                  child: Container(
                    decoration: BoxDecoration(
                                                  ...
                                                  Row(
                                                    ...
                                                    children: [
                                                      ...
                                                      Expanded(
                                                        child: Text(
                                                          getTranslated(
                                                            'save_the_necessary_information',
                                                            context,
                                                          ),
                                                          style: poppinsSemiBold
                                                              .copyWith(
