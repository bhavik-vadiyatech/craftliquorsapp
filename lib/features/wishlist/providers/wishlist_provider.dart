...
import 'package:craft_discount_liquors/features/wishlist/domain/models/wishlist_model.dart';
...
class WishListProvider extends ChangeNotifier {
  ...
  void addToWishList(Product product, BuildContext context) async {
    ...
    notifyListeners();
    ApiResponseModel apiResponse = await wishListRepo!.addWishList([
      product.id,
    ]);
  ...
  void removeFromWishList(Product product, BuildContext context) async {
    ...
    _wishIdList.remove(product.id);
    notifyListeners();
    ApiResponseModel apiResponse = await wishListRepo!.removeWishList([
      product.id,
    ]);
    if (apiResponse.response != null &&
        apiResponse.response!.statusCode == 200) {
  ...
  Future<void> getWishListProduct() async {
    ...
    DataSyncHelper.fetchAndSyncData(
      fetchFromLocal: () =>
          wishListRepo!.getWishList(source: DataSourceEnum.local),
      fetchFromClient: () =>
          wishListRepo!.getWishList(source: DataSourceEnum.client),
      onResponse: (data, _) {
        _wishList = [];
