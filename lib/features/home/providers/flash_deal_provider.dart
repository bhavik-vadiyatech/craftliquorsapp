  ...
  Future<void> getFlashDealProducts(int offset, {bool isUpdate = true}) async {
    ...
    if (offset == 1) {
      DataSyncHelper.fetchAndSyncData(
        fetchFromLocal: () => productRepo.getFlashDeal(
          offset: offset,
          source: DataSourceEnum.local,
        ),
        fetchFromClient: () => productRepo.getFlashDeal(
          offset: offset,
        ...
        onResponse: (data, _) {
          ...
          if (_flashDealModel?.flashDeal?.endDate != null) {
            DateTime endTime = DateConverterHelper.isoStringToLocalDate(
              _flashDealModel!.flashDeal!.endDate!,
      ...
      ApiResponseModel? response = await productRepo.getFlashDeal(
        offset: offset,
        source: DataSourceEnum.client,
      );
      if (response.response?.statusCode == 200) {
        if (offset == 1) {
          ...
          _flashDealModel?.offset = ProductModel.fromJson(
            response.response?.data,
          ).offset;
          _flashDealModel?.totalSize = ProductModel.fromJson(
            response.response?.data,
          ).totalSize;
          _flashDealModel?.flashDeal = ProductModel.fromJson(
            response.response?.data,
          ).flashDeal;
          _flashDealModel?.products?.addAll(
