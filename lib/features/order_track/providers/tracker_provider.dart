...
class TrackerProvider with ChangeNotifier {
  ...
  Future<void> startLocationService({
    ...
    _locationServiceTimer = Timer.periodic(const Duration(seconds: 10), (
      timer,
    ) {
      getDeliveryManData(
        deliverymanId: deliverymanId,
  ...
  Future<void> getDeliveryManData({
    ...
    ApiResponseModel apiResponse = await trackerRepo!.getDeliveryManData(
      deliverymanId: deliverymanId,
      orderId: orderId,
    );
      ...
      if (mapController != null) {
        ...
        if (_deliveryManModel != null) {
          deliverymanLocation = LatLng(
            _deliveryManModel?.latitude ?? 23.0,
  ...
  void _mapBound({
    ...
    Future.delayed(const Duration(milliseconds: 100), () {
      controller?.animateCamera(
        CameraUpdate.newLatLngBounds(adjustedBounds, 100),
      );
    });
  }
  ...
  Future<BitmapDescriptor> _convertAssetToUnit8List(
    String imagePath, {
    double height = 50,
    double width = 50,
  }) async {
