...
import 'package:craft_discount_liquors/features/payment/widgets/cancel_dialog_widget.dart';
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      onPopInvokedWithResult: ((_, _) =>
          _exitApp(context).then((value) => value!)),
      child: Scaffold(
        body: Center(
          child: Stack(
            children: [
  ...
  void _pageRedirect(String url) {
    if (_canRedirect) {
      bool checkedUrl =
          ...
          url.contains('${AppConstants.baseUrl}${RouteHelper.orderDetails}'));
      ...
      bool isWallet = url.contains(
        '${AppConstants.baseUrl}${RouteHelper.wallet}',
      );
      bool isOrderDetails = url.contains(
      ...
      if (isSuccess) {
        ...
        if (isWallet) {
          ...
          if (token.isNotEmpty) {
            ...
            String placeOrderString = utf8.decode(
              base64Url.decode(
                orderProvider.getPlaceOrder()!.replaceAll(' ', '+'),
              ),
            );

            String decodeValue = utf8.decode(
              base64Url.decode(token.replaceAll(' ', '+')),
            );
            String paymentMethod = decodeValue.substring(
              0,
              decodeValue.indexOf('&&'),
            );
            String transactionReference = decodeValue.substring(
              decodeValue.indexOf('&&') + '&&'.length,
              decodeValue.length,
            );
