...
import 'package:craft_discount_liquors/common/models/place_details_model.dart';
...
import 'package:craft_discount_liquors/features/address/domain/models/prediction_model.dart';
...
class LocationProvider with ChangeNotifier {
  ...
  Future<String> getAddressFromGeocode(LatLng latLng) async {
    ...
    if (response.response!.statusCode == 200 &&
      ...
      address = response.response!.data['results'][0]['formatted_address']
          .toString();
    } else {
  ...
  Future<List<PredictionModel>> searchLocation(
    ...
    if (text.isNotEmpty) {
      ...
      if (response.response?.statusCode == 200) {
        _predictionList = [];
        response.response?.data['suggestions'].forEach(
          (prediction) =>
