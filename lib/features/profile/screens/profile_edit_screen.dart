...
import 'package:craft_discount_liquors/features/profile/widgets/profile_menu_web_widget.dart';
    ...
    if (profileProvider.userInfoModel == null) {
      await profileProvider.getUserInfo(true, isUpdate: false).then((_) {
        ...
        if (profileProvider.userInfoModel?.phone?.isNotEmpty ?? false) {
          ...
          _phoneController?.text = PhoneNumberCheckerHelper.getPhoneNumber(
            profileProvider.userInfoModel?.phone ?? '',
            countryCode ?? '',
          )!;
...
void showLoader(BuildContext context) {
  showDialog(
    ...
    builder: (BuildContext context) {
      return const CustomLoaderWidget(color: Colors.white);
      // return Dialog(
      //
