...
import 'package:craft_discount_liquors/helper/file_validation_helper.dart';
...
class ReviewProvider extends ChangeNotifier {
  ...
  void initRatingData(List<OrderDetailsModel> orderDetailsList) {
    ...
    for (int i = 0; i < orderDetailsList.length; i++) {
      ...
      _reviewImageList.add(
        ProductWiseReviewImage(orderDetailsList[i].productId!, []),
      );
    }
  }
  ...
  Future<ResponseModel> submitReview(
    ...
    ApiResponseModel response = await orderRepo!.submitReview(
      reviewBody,
      _reviewImageList[index].image,
    );
  ...
  Future<ResponseModel> submitDeliveryManReview(
    ...
    _isLoading = true;
    notifyListeners();
    ApiResponseModel response = await orderRepo!.submitDeliveryManReview(
      reviewBody,
    );
    ResponseModel responseModel;
    if (response.response != null && response.response!.statusCode == 200) {
  ...
  void pickImage(BuildContext context, bool isRemove, int productId) async {
    ...
    try {
      ...
      if (pickedImage != null) {
        ...
        for (ProductWiseReviewImage productWiseReviewImage
            in _reviewImageList) {
