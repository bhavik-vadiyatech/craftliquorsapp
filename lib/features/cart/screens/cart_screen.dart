...
import 'package:craft_discount_liquors/features/cart/widgets/cart_button_widget.dart';
import 'package:craft_discount_liquors/features/cart/widgets/cart_product_list_widget.dart';
...
class _CartScreenState extends State<CartScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return CustomPopScopeHandelDeepLinkWidget(
      child: Scaffold(
        appBar:
            ResponsiveHelper.isMobilePhone() || ResponsiveHelper.isTab(context)
            ? null
              ...
              return Consumer<CartProvider>(
                builder: (context, cart, child) {
                  ...
                  for (var cartModel in cart.cartList) {
                    ...
                    tax =
                        tax +
                        ((cartModel.tax ?? 0) * (cartModel.quantity ?? 0));
                  }

                  double subTotal =
                      itemPrice +
                      ((configModel?.isVatTexInclude ?? false) ? 0 : tax);
                  bool isFreeDelivery =
