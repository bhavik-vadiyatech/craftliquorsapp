...
import 'package:craft_discount_liquors/features/order/domain/models/review_body_model.dart';
...
class OrderRepo {
  ...
  Future<ApiResponseModel> getDeliveryManData({
    String? deliverymanId,
    String? orderId,
  }) async {
    try {
      final response = await dioClient!.get(
        '${AppConstants.lastLocationUri}$deliverymanId&order_id=$orderId',
      );
      return ApiResponseModel.withSuccess(response);
    } catch (e) {
      return ApiResponseModel.withError(ApiErrorHandler.getMessage(e));
  ...
  List<String> getDateList() {
    ...
    dates.add(
      DateConverterHelper.slotDate(DateTime.now().add(const Duration(days: 1))),
    );
    dates.add(
      DateConverterHelper.slotDate(DateTime.now().add(const Duration(days: 2))),
    );
  ...
  Future<ApiResponseModel> storeOfflineData({
    String? orderId,
    String? isPartial,
  ...
  Future<ApiResponseModel> switchPaymentMethod({
  ...
  Future<ApiResponseModel> getDigitalPaymentResponse({
  ...
  Future<ApiResponseModel> placeDigitalOrder(PlaceOrderModel orderBody) async {
    try {
      final response = await dioClient!.post(
        AppConstants.placeDigitalOrder,
        data: orderBody.toJson(),
      );
      return ApiResponseModel.withSuccess(response);
    } catch (e) {
