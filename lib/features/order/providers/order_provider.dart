...
import 'package:craft_discount_liquors/common/models/cart_model.dart';
import 'package:craft_discount_liquors/common/models/config_model.dart';
import 'package:craft_discount_liquors/features/checkout/widgets/incomplete_offline_payment_dialog.dart';
import 'package:craft_discount_liquors/features/order/domain/models/distance_model.dart';
import 'package:craft_discount_liquors/features/order/domain/models/offline_payment_model.dart';
import 'package:craft_discount_liquors/features/order/domain/models/order_details_model.dart';
...
import 'package:craft_discount_liquors/common/models/response_model.dart';
...
import 'package:craft_discount_liquors/features/order/domain/models/delivery_man_model.dart';
...
class OrderProvider extends ChangeNotifier {
  ...
  void validateSlot(List<TimeSlotModel>? slots, int dateIndex) {
    ...
    if (dateIndex == 0) {
      ...
      for (var slot in slots!) {
        DateTime time = DateConverterHelper.stringTimeToDateTime(
          slot.endTime!,
  ...
  Future<List<OrderDetailsModel>?> getOrderDetails({
    ...
    totalPrice = 0;
    notifyListeners();
    ApiResponseModel apiResponse = await orderRepo!.getOrderDetails(
      orderID,
      phoneNumber,
    );
    _isLoading = false;
  ...
  Future<void> getDeliveryManData(
    ...
    ApiResponseModel apiResponse = await orderRepo!.getDeliveryManData(
  ...
  Future<bool> getDistanceInMeter(
    ...
    ApiResponseModel response = await orderRepo!.getDistanceInMeter(
      originLatLng,
      destinationLatLng,
    );
    try {
  ...
  void storeOfflineData({
    ...
    ApiResponseModel apiResponse = await orderRepo!.storeOfflineData(
      orderId: orderId,
      isPartial: isPartial,
  ...
  void revalidateWalletPayment({
    required double totalAmount,
    required double walletBalance,
    bool isUpdate = true,
  }) {
    if (_paymentMethodIndex == 0 || _partialAmount != null) {
      // Case 1: Total exceeds wallet balance
      if (totalAmount > walletBalance) {
        _partialAmount = walletBalance;
        _paymentMethodIndex = 0;
        // Keep wallet selected, clear only secondary payment methods
        _selectedPaymentMethod = PaymentMethod(
          getWayTitle: getTranslated('wallet_payment', Get.context!),
          getWay: 'wallet_payment',
          type: 'wallet_payment',
      ...
      // Case 2: Total fits within wallet
  ...
  Future<List<CartModel>?> reorderProduct(String orderId) async {
    ...
    List<OrderDetailsModel>? orderDetailsList = await getOrderDetails(
      orderID: orderId,
    );
    reOrderCartList = [];
    ...
    for (OrderDetailsModel orderDetails in orderDetailsList ?? []) {
      ...
      if (orderDetails.formattedVariation != null) {
        selectVariationType = OrderHelper.getVariationValue(
          orderDetails.formattedVariation,
        );
      }
      ...
      try {
        ...
        showCustomSnackBarHelper(
          getTranslated('this_product_is_currently_unavailable', Get.context!),
        );
      }
  ...
  void manageDialog() async {
    if (Provider.of<AuthProvider>(Get.context!, listen: false).isLoggedIn()) {
      Provider.of<ProfileProvider>(
        Get.context!,
        listen: false,
      ).getUserInfo(true, isUpdate: true).then((value) async {
        var userData = Provider.of<ProfileProvider>(
          Get.context!,
          listen: false,
        ).userInfoModel;
        ...
        if (Provider.of<AuthProvider>(
          ...
          await ResponsiveHelper()
              .showDialogOrBottomSheet(
                Get.context!,
                IncompleteOfflinePaymentDialog(
                  order: userData.lastIncompleteOfflineBooking,
                ),
                isScrollControlled: true,
              )
              .then((value) {
                setLastIncompleteOfflineBookingId(
                  userData.lastIncompleteOfflineBooking?.id ?? 0,
                );
              });
        }
      });
  ...
  Future<void> switchPaymentMethod({
    ...
    if (apiResponse.response != null &&
      ...
      showCustomSnackBarHelper(
        getTranslated('payment_method_switched_successfully', Get.context!),
        isError: false,
      );
    } else {}
  ...
  Future<PaymentResponseModel?> getDigitalPaymentResponse({
    ...
    ApiResponseModel apiResponse = await orderRepo!.getDigitalPaymentResponse(
      transactionId: transactionId,
    );

    PaymentResponseModel? paymentResponseModel;
    if (apiResponse.response.statusCode == 200) {
      paymentResponseModel = PaymentResponseModel.fromJson(
  ...
  Future<String?> placeDigitalOrder(
    BuildContext context,
    PlaceOrderModel placeOrderBody,
  ) async {
    _isLoading = true;
    ...
    if (apiResponse.response?.statusCode == 200 &&
        apiResponse.response.data['redirect_link'] != null) {
